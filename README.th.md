...
ใช้โทเคนน้อยลงบน ChatGPT, Claude และ Gemini เวลาพิมพ์ภาษาไทย (และภาษาเวียดนาม อินโดนีเซีย)</p>
...
> **v0.2 ยังเป็นเวอร์ชันช่วงแรก** ใช้ได้กับทั้งสามเว็บแล้ว และอยากได้ความเห็นจากการใช้งานจริงมาก
...
- **รองรับไทย เวียดนาม และอินโดนีเซีย** เลือกชุดกฎให้อัตโนมัติจากข้อความที่พิมพ์ และปิดกฎที่ไม่ต้องการได้ทีละข้อในป็อปอัป
- **คีย์ลัด**: <kbd>Alt</kbd>+<kbd>Shift</kbd>+<kbd>K</kbd> เปิดหน้าตัวอย่างบีบข้อความ (เปลี่ยนได้ที่ `chrome://extensions/shortcuts`)
- **สถิติ 7 วัน**: ป็อปอัปแสดงโทเคนที่ประหยัดได้ในแต่ละวัน เก็บในเครื่องคุณเท่านั้น
- **ยังใช้ได้แม้เว็บเปลี่ยนหน้าตา**: ถ้าแถบหาที่วางเดิมไม่เจอ จะมีปุ่มเล็ก **TadTingPai · บีบข้อความ** ขึ้นข้างช่องแชทแทน
  และในป็อปอัปมีลิงก์ **แจ้งว่าเว็บเปลี่ยน** ที่กรอกข้อมูลให้เอง (ไม่มีข้อความที่คุณพิมพ์แนบไปด้วย)
...
6. กดไอคอน TadTingPai บนแถบเครื่องมือ เพื่อดูยอดโทเคนที่ประหยัดได้ (รวมและ 7 วันล่าสุด) เลือกเว็บที่จะให้แถบขึ้น
   เปิด/ปิดกฎการบีบทีละข้อ และสลับภาษาหน้าจอ ไทย/English
7. ถนัดคีย์บอร์ด? กด <kbd>Alt</kbd>+<kbd>Shift</kbd>+<kbd>K</kbd> เพื่อเปิดหน้าตัวอย่างบีบข้อความได้เลย
...
src/
  ...
  languages/     ชุดภาษาในรูปข้อมูล (th, vi, id: src/languages/<code>/rules.ts)
...
ตัวบีบข้อความไม่ผูกกับภาษาใดภาษาหนึ่ง วลีจะถูกตัดก็ต่อเมื่อ
...
ภาษาเวียดนามและอินโดนีเซียเว้นวรรคระหว่างคำ ชุดกฎของสองภาษานี้จึงตั้ง `wordSpacing` ไว้ ทำให้ "ท้ายวลี" หมายถึงก่อนเครื่องหมายวรรคตอนหรือขึ้นบรรทัดใหม่
ไม่ใช่ทุกช่องว่าง และจับคำโดยไม่สนตัวพิมพ์เล็กใหญ่ ภาษาอินโดนีเซียใช้ตัวอักษรละตินเหมือนภาษาอังกฤษ จึงจะเลือกชุดนี้ก็ต่อเมื่อเจอคำอินโดนีเซียที่พบบ่อย (`markers`)
ข้อความภาษาอังกฤษจะไม่ถูกบีบ
...
- [ ] ป็อปอัปแสดง "แถบเครื่องมือปกติ" ของทุกเว็บที่เปิด (ไม่ใช่ "ใช้ปุ่มลอยแทน")
- [ ] กด <kbd>Alt</kbd>+<kbd>Shift</kbd>+<kbd>K</kbd> แล้วหน้าตัวอย่างเปิดขึ้น

ถ้าเว็บไหนพัง ส่วนใหญ่แก้แค่ selector เดียวใน `src/adapters/<site>.ts` ภาพสำหรับ Chrome Web Store:
`npm run build && node scripts/store-screenshots.mjs` ส่วนข้อความหน้าร้านอยู่ที่ [docs/store/listing.md](docs/store/listing.md)
