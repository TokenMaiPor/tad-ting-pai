...
export function createToolbar(initialLang: UiLang, handlers: ToolbarHandlers): Toolbar {
  ...
  const bar = h(
    'div',
    { class: 'toolbar', role: 'region' },
    h('span', { class: 'count' }, figure, note),
    status,
    h('div', { class: 'actions' }, compressBtn, translateBtn),
  );
  ...
  function render() {
    ...
    compressBtn.textContent = t(lang, 'toolbar.compress');
